...
def find_top_rpn_proposals(
    ...
    for n, image_size in enumerate(image_sizes):
        ...
        # During training, topk is over the proposals from *all* JPEGImages in the training batch.
...
def add_ground_truth_to_proposals(
    ...
    """
    Call `add_ground_truth_to_proposals_single_image` for all JPEGImages.
    ...
    if len(proposals) != len(gt):
        raise ValueError("proposals and gt should have the same length as the number of JPEGImages!")
    if len(proposals) == 0:
        return proposals
