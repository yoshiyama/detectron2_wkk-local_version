...
class FixedSizeCrop(Augmentation):
    ...
    def __init__(
        ...
        Args:
            ...
            pad: if True, will pad JPEGImages smaller than `crop_size` up to `crop_size`
...
class RandomSaturation(Augmentation):
    ...
    Input JPEGImages are assumed to have 'RGB' channel order.
    ...
    def get_transform(self, image):
        assert image.shape[-1] == 3, "RandomSaturation only works on RGB JPEGImages"
        w = np.random.uniform(self.intensity_min, self.intensity_max)
...
class RandomLighting(Augmentation):
    ...
    def get_transform(self, image):
        assert image.shape[-1] == 3, "RandomLighting only works on RGB JPEGImages"
...
class MinIoURandomCrop(Augmentation):
    ...
    def get_transform(self, image, boxes):
        """Call function to crop JPEGImages and bounding boxes with minimum IoU
        constraint.
