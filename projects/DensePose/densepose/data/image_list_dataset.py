...
class ImageListDataset(Dataset):
    """
    Dataset that provides JPEGImages from a list.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            category_list (Union[str, List[str], None]): list of animal categories for
                each image. If it is a string, or None, this applies to all JPEGImages
        """
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        """
        Gets selected JPEGImages from the list

        Args:
            idx (int): video index in the video list file
        Returns:
            A dictionary containing two keys:
                JPEGImages (torch.Tensor): tensor of size [N, 3, H, W] (N = 1, or 0 for _EMPTY_IMAGE)
        ...
        try:
            ...
            return {"JPEGImages": image, "categories": categories}
        except (OSError, RuntimeError) as e:
            logger = logging.getLogger(__name__)
            logger.warning(f"Error opening image file container {fpath}: {e}")

        return {"JPEGImages": self._EMPTY_IMAGE, "categories": []}
