...
class InferenceBasedLoader:
    ...
    Data loader based on results inferred by a model. Consists of:
     - a data loader that provides batches of JPEGImages
    ...
    def __init__(
        ...
        Args:
          ...
          data_loader (Iterable[List[Dict[str, Any]]]): iterable that provides
            dictionaries with "JPEGImages" and "categories" fields to perform inference on
          ...
          data_filter (Callable: ModelOutput -> ModelOutput): filter
              ...
              (optional, default: None)
          shuffle (bool): if True, the input JPEGImages get shuffled
          batch_size (int): batch size for the produced annotation data
          inference_batch_size (int): batch size for input JPEGImages
    ...
    def __iter__(self) -> Iterator[List[SampledData]]:
        for batch in self.data_loader:
            ...
            images_and_categories = [
                ...
                for image, category in zip(element["JPEGImages"], element["categories"])
            ]
    ...
    def _produce_data(
        ...
        """
        Produce batches of data from JPEGImages

        Args:
          images_and_categories (List[Tuple[torch.Tensor, Optional[str]]]):
            list of JPEGImages and corresponding categories to process
