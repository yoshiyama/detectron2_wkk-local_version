...
def get_texture_atlas(path: Optional[str]) -> Optional[np.ndarray]:
    ...
    # Reading JPEGImages like that downsamples 16-bit JPEGImages to 8-bit
    # If 16-bit JPEGImages are needed, we can replace that by cv2.imread with the
    # cv2.IMREAD_UNCHANGED flag (with cv2 we also need it to keep alpha channels)
    # The rest of the pipeline would need to be adapted to 16-bit JPEGImages too
